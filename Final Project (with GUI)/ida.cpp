...
void ida::build(string input, int h)
{
        read(h, input);
	int count;
	...
	while(check){
		...
		if(check){
			...
			cout << "incrementing to " << nextf<<endl;
		//int a;
		//cin >> a;
			root = NULL;
			fflush(stdout);
...
void ida::read(int algo, string conf) {
	...
	for (int i=0; i<9; i++)
		t1->state.push_back(initial[i]);
	t1->cost = costCalculator(t1,algorithm);
	fvalue = t1->cost;
	cout << "fvalue initial (MD) = "<<fvalue<<endl;

}
void ida::makeRoot(vector<int> state) {
	tree_node* t1 = new tree_node;
...
int ida::costCalculator(tree_node* t, int j) {
	int cost = 0;
	if(j==1){
		for (int i=0; i<t->state.size(); i++) {
			//cout << "Number "<<t->state[i]<<" at position "<<i<<endl;
			//cout<<"Distance = "<<distanceArray[t->state[i]][i]<<endl;
			//cout << "distanceArray["<<t->state[i]<<"]["<<i<<"] ="<<distanceArray[t->state[i]][i]<<endl;
...
void ida::makeMove(tree_node* curr, tree_node* t, char direction) {
	int temp, index;
	bool badMove = false;
...
void ida::insert(tree_node* curr) {
	tree_node* t1 = new tree_node;
	makeMove(curr, t1, 'u');
	...
	if ((curr->link1 == NULL) && (t1->state.size() > 0)) {
		curr->link1 = t1;
		if (!success){
                        if (t1->cost <= fvalue) tacky.push_back(curr->link1);
			else {
				if (t1->cost<nextf) nextf = t1->cost;
			}
		}
	}
	else
	...
	if ((curr->link2 == NULL) && (t2->state.size() > 0)) {
		curr->link2 = t2;
		if (!success) {
                        if (t2->cost <= fvalue) tacky.push_back(curr->link2);
			else {
				if (t2->cost<nextf) nextf = t2->cost;
			}
		}
	}
	else
	...
	if ((curr->link3 == NULL) && (t3->state.size() > 0)) {
		curr->link3 = t3;
		if (!success) {
                        if (t3->cost <= fvalue) tacky.push_back(curr->link3);
			else {
				if (t3->cost<nextf) nextf = t3->cost;
			}
		}
	}
	else
	...
	if ((curr->link4 == NULL) && (t4->state.size() > 0)) {
		curr->link4 = t4;
		if (!success) {
                        if (t4->cost <= fvalue) tacky.push_back(curr->link4);
			else {
                                if (t4->cost<nextf) nextf = t4->cost;
			}
		}
	}
	else
...
void ida::checkSuccess(tree_node* curr) {
	bool    pass1 = true,
	pass2 = true,
	pass3 = true,
	pass4 = true;
